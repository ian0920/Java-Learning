...
public class Cart {
    ...
    public void addItem (InventoryItem item, int qty){
        products.merge(item.getProduct().sku(), qty, Integer::sum); //(oldValue, newValue) -> oldValue + newValue lambda expression

        if(!item.reserveItem(qty)){
            System.out.println("oh, something went wrong, could not add item");
        }
    ...
    public void removeItem (InventoryItem item, int qty){
        ...
        if(current <= qty){
            ...
            System.out.printf("Item [%s] removed from cart ", item.getProduct().name());
