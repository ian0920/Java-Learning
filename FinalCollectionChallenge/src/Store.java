...
public class Store {
    ...
    public static void main(String[] args) {
        ...
        myStore.manageStoreCart();
        myStore.listProductByCategory(false, true);
    ...
    private void manageStoreCart(){
        ...
        if (!checkOutCart(cart3)){
            System.out.println("something went wrong, couldn't check");
        }
    ...
    private void abandonCart(){
        ...
        while((abandonCart = oldCart.pollFirst()) != null){
    ...
    private void listProductByCategory(){
        ...
        listProductByCategory(true, false);

    }
    private void listProductByCategory(boolean includeHeader, boolean includeDetail){
    ...
    private void stockAisles(){
        ...
        for (InventoryItem item : inventory.values()){
            Category aisles = item.getProduct().category();

            Map <String, InventoryItem> productMap = aisleInventory.get(aisles);
            if(productMap == null){
                productMap = new TreeMap<>();
            }
            productMap.put(item.getProduct().name(), item);
            aisleInventory.putIfAbsent(aisles, productMap);
